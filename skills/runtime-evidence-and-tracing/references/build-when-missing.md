...
Use this when runtime validation is not auditable. Pattern sources include repositories with run artifacts, request IDs, browser evidence, trace scripts, log queries, and provider-blocker classification.
...
- `scripts/harness/trace_run.py`, `scripts/harness/doctor.py`, or a documented manual collection command.
...
4. Define failure classification: code regression, environment unavailable, provider blocker, data missing, not evaluable.
...
- Confirm the same run ID appears in available frontend, backend, worker, or provider evidence.
- Confirm artifacts are referenced but not committed when large or sensitive.

## Do Not Include

- Tokens, real user identifiers, phone numbers, provider keys, or sensitive payloads.
- Claims of browser/device validation without evidence of the actual engine used.
