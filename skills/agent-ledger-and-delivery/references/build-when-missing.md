...
Use this when agent work has no durable delivery record. Pattern sources include `agent_chats/` and `agents_chat/` directories with date-based records, validation sections, and linked commits.
...
3. Define required sections: user prompt, goals, changes, validation, risks or next steps, linked commits.
4. Define coupling: per commit, per PR, per session, or per milestone.
